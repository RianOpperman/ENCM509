...
# Run with OUR prerecorded data
# process_data(datafiles, use_prerecorded=True)


# process_data(...,  use_prerecorded = True or False)
...
NUMBER_TIMESTEPS = 150
# Make sure NUMBER_FEATURES and NUMBER_OUTPUTS are defined appropriately
...
def build_model():
    ...
    model.add(Bidirectional(layers.LSTM(256, return_sequences=True, recurrent_dropout=0.2)))
    model.add(Bidirectional(layers.LSTM(128, recurrent_dropout=0.2)))
...
history = model.fit(X_train, y_train, epochs=100, batch_size=len_train, callbacks=[early_stopping, reduce_lr])
...
# testing the model
test_loss, test_acc, percision, recall = model.evaluate(X_test, y_test, batch_size=len_train)
...
# model.save("../model/gesture_model_bidirectional.h5")
